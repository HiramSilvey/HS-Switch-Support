...
volatile uint8_t usb_configuration =
    0;  // non-zero when USB host as configured device
volatile uint8_t usb_high_speed =
    0;  // non-zero if running at 480 Mbit/sec speed
...
FLASHMEM void usb_init(void) {
  ...
  CCM_CCGR6 |=
      CCM_CCGR6_USBOH3(CCM_CCGR_ON);  // turn on clocks to USB peripheral
  ...
  // Before programming this register, the PHY clocks must be enabled in
  // registers USBPHYx_CTRLn and CCM_ANALOG_USBPHYx_PLL_480_CTRLn.
...
static void isr(void) {
  ...
  if (status & USB_USBSTS_URI) {  // page 3164
    USB1_ENDPTSETUPSTAT =
        USB1_ENDPTSETUPSTAT;  // Clear all setup token semaphores
    USB1_ENDPTCOMPLETE =
        USB1_ENDPTCOMPLETE;  // Clear all the endpoint complete status
...
// configure all other endpoints
    ...
    case 0x0082:  // GET_STATUS (endpoint)
      ...
      reply_buffer[0] = 0;
      reply_buffer[1] = 0;
      if ((dir && (ctrl & USB_ENDPTCTRL_TXS)) ||
          (!dir && (ctrl & USB_ENDPTCTRL_RXS))) {
        reply_buffer[0] = 1;
      }
      endpoint0_transmit(reply_buffer, 2, 0);
      return;
    ...
    defined(JOYSTICK_INTERFACE) || defined(NSGAMEPAD_INTERFACE)
...
static void endpoint0_complete(void) {
  ...
  if (setup.wRequestAndType == 0x2021 &&
      setup.wIndex == CDC2_STATUS_INTERFACE) {
...
#ifdef CDC3_STATUS_INTERFACE
  if (setup.wRequestAndType == 0x2021 &&
      setup.wIndex == CDC3_STATUS_INTERFACE) {
...
static void usb_endpoint_config(endpoint_t *qh, uint32_t config,
                                void (*callback)(transfer_t *)) {
...
static void schedule_transfer(endpoint_t *endpoint, uint32_t epmask,
                              transfer_t *transfer) {
...
static void run_callbacks(endpoint_t *ep) {
  ...
  // count how many transfers are completed, then remove them from the
  // endpoint's list
  uint32_t count = 0;
  transfer_t *t = first;
  while (1) {
